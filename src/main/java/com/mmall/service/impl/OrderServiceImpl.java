...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId){
        ...
        if(order==null){
            return ServerResponse.createByErrorMessage("生成订单为空");
        }
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList){
        ...
        for(Cart cartItem:cartList){
            ...
            if(Const.ProductStatusEnum.ON_SALE.getCode()!=product.getStatus()){
                return ServerResponse.createByErrorMessage("产品"+product.getName()+"不是贩卖状态");
            }
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path){
        ...
        //从order_item中查出明细并填充goodsdetail
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(PropertiesUtil.getProperty("alipay.callback.url"))//支付宝服务器主动通知商户服务器指定页面的https回调地址
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                //二维码路径
                String qrPath = String.format(path+"/qr-%s.qng",response.getOutTradeNo());
                String qrFileName = String.format("qr-%s.qng",response.getOutTradeNo());
                ZxingUtils.getQRCodeImge(response.getQrCode(),256,qrPath);
                File targetFile = new File(path,qrFileName);
                FTPUtil.uploadFileBatch(Lists.newArrayList(targetFile));
    ...
    private void dumpResponse(AlipayResponse response){
        if(response!=null){
            ...
            if(StringUtils.isNoneEmpty(response.getSubCode())){
                log.info(String.format("code:%s,msg:%s",response.getCode(),response.getMsg()));
            }
